...
# base::ifelse() returns logical(0) on empty inputs; use for character columns.
char_ifelse = function(test, yes, no) {
  dplyr::if_else(test, as.character(yes), as.character(no))
}

# Design / auxiliary CSVs are wide with many empty or mixed columns; read everything as
# character (downstream code coerces types) to avoid noisy vroom type-guessing warnings.
# These files also carry a harmless trailing comma on each row (one extra empty column vs the
# header); drop that column and ignore the resulting non-fatal parse warning so runs stay clean.
read_design_csv = function(path) {
  out = suppressWarnings(
    readr::read_csv(path, col_types = readr::cols(.default = readr::col_character()))
  )
  drop = names(out)[grepl("^(\\.\\.\\.|X)[0-9]+$", names(out)) & vapply(out, function(col) all(is.na(col)), logical(1))]
  if (length(drop)) {
...
# Zero-row scaffold when a session folder has no exports yet.
empty_morph_session_raw = function() {
  tibble(
    participant = character(),
    !!birth_col := character(),
    date = character(),
    running_index_in_file = integer()
  )
}

# Zero-row export matching cols_keep (after preprocessing pipeline).
empty_morph_session_export = function(session_num = NA_integer_) {
  out = tibble(
    participant = character(),
    ...
    previous_encounters = double(),
    target_video = character(),
    foil_video = character(),
    audio = character(),
    t_start = double(),
    t_end = double(),
  ...
  if (!is.na(session_num)) {
    out = out |> mutate(session = as.integer(session_num))
  }
  out
}

# Load CSVs from a Morph session folder and bind into one data frame (same column types as session1).
...
read_morph_session = function(session_dir) {
  files = list.files(path = session_dir, pattern = "*.csv", full.names = TRUE)
  if (!length(files)) {
    warning("read_morph_session: no CSV files in ", session_dir)
    return(empty_morph_session_raw())
  }
  lst = lapply(files, function(f) {
    readr::read_csv(f, show_col_types = FALSE, na = c("", "NA"))
  })
  lst = lapply(lst, function(df) {
    # optionOrder is comma-separated indices in Psychopy; read_csv guesses char vs double per file
    out = df |> mutate(participant = as.character(.data$participant))
    if (birth_col %in% names(out)) {
      out = out |> mutate(!!birth_col := as.character(.data[[birth_col]]))
    } else {
      out = out |> mutate(!!birth_col := NA_character_)
    }
    out |>
...
consolidate_running_index_after_read = function(df) {
  if (!nrow(df)) {
    return(df |> mutate(running_index = integer()))
  }
...
join_canonical = function(df) {
  if (!nrow(df)) {
    return(df |> mutate(participant_canonical = character()))
  }
  bc = birth_col
  df |>
    ...
    mutate(
      ...
      participant_alp_key = {
        ...
        char_ifelse(is_alp_id(p), p, NA_character_)
      }
    ) |>
...
narrow_morph_session = function(df) {
  na_col = function(nm) {
    if (nm %in% names(df)) {
      df[[nm]]
    } else {
...
join_trial_difficulties = function(df, wo_session) {
  if (!nrow(df)) {
    return(df |> mutate(trial_id_global = double(), sentence_complexity = double()))
  }
  ...
  sk_row = char_ifelse(is.na(lab), NA_character_, normalize_chunk_label(lab))
...
finalize_export_identity = function(df) {
  ...
  birth_date_out = char_ifelse(is.na(bd), NA_character_, format(bd, "%Y-%m-%d"))

  # Keep canonical ALPxxxx IDs. For other Psychopy codes, keep the raw code only when there is
  # no usable birth date either, so numeric codes still distinguish participants (rows always kept).
  keep_raw_code = !is_alp_id(participant_out) & is.na(bd)
  participant_out = char_ifelse(
    is_alp_id(participant_out) | keep_raw_code,
    participant_out,
    NA_character_
  )
...
# Stable analysis ID: participant code (ALP, else raw Psychopy code) when present;
# otherwise missing-YYYY-MM-DD from birth_date, else missing-unknown.
add_participant_unique_id = function(df) {
  ...
  p[is.na(p)] = ""
  bd[is.na(bd)] = ""
  has_code = nzchar(p) & !toupper(p) %in% c("NA", "NAN")
  uid = ifelse(
    has_code,
    p,
...
process_morph_session = function(
  df,
  session_num,
  eligible_ids = NULL,
  eligible_label = ""
  ...
  if (!nrow(df)) {
    return(empty_morph_session_export(session_num))
  }
  out = narrow_morph_session(df) |>
    add_task_type() |>
    enrich_design_trials(session_num) |>
    join_sentence_encounters(session_num) |>
    collapse_trial_columns() |>
    rename_experiment_name() |>
    rename_previous_encounters() |>
    na_zero_previous_encounters_illegal_chunk() |>
    add_trial_type_legality() |>
    join_trial_difficulties(session_num) |>
    mutate(session = session_num)
  if (!is.null(eligible_ids)) {
    out = filter_participant_canonical_in(out, eligible_ids, eligible_label)
  }
  out |>
    finalize_export_identity() |>
    add_reaction_time() |>
...
session1 = read_morph_session("data/MorphSession1") |> consolidate_running_index_after_read()
session2 = read_morph_session("data/MorphSession2") |> consolidate_running_index_after_read()
session3 = read_morph_session("data/MorphSession3") |> consolidate_running_index_after_read()
# Distinct (participant, birth) pairs across all Morph sessions — union-find merges rows linked by
# same ALP code or same parsed birth date so ALP propagates across sessions / numeric codes.
session_identity_pairs = function(df) {
  need = c("participant", birth_col)
  if (!all(need %in% names(df))) {
    return(tibble(participant = character(), !!birth_col := character()))
  }
  df |> select(all_of(need)) |> distinct()
}

all_pairs = bind_rows(
  session_identity_pairs(session1),
  session_identity_pairs(session2),
  session_identity_pairs(session3)
) |>
  distinct()
...
session1 = process_morph_session(session1, 1L)
session2 = process_morph_session(session2, 2L, cid_session2_eligible, "cid_session2_eligible")
session3 = process_morph_session(session3, 3L, cid_session3_eligible, "cid_session3_eligible")
...
session_1_filter_participant = character(0)
session_1_filter_birth_date = character(0)
session_2_filter_participant = character(0)
session_2_filter_birth_date = character(0)
session_3_filter_participant = character(0)
session_3_filter_birth_date = character(0)
...
all_sessions = dplyr::bind_rows(session1, session2, session3) |>
...
write_csv(all_sessions, "results/BrocantoHPilot1MorphData.csv", na = "")
