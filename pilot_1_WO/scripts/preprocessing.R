...
session1 = read_wo_session("data/WOSession1") |> consolidate_running_index_after_read()
session2 = read_wo_session("data/WOSession2") |> consolidate_running_index_after_read()
session3 = read_wo_session("data/WOSession3") |> consolidate_running_index_after_read()
...
all_pairs = bind_rows(
  ...
  session3 |> select(participant, all_of(birth_col)) |> distinct()
) |>
  distinct()
